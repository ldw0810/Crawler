...
;	if (checked.length) {
    if(+$("#wholeCheckBox_title").attr("isoption")){
		...
		console.log(str);delParam = {"id":str,"action":"1"}
		 ajaxFunction("/taskMgmt", "POST", "JSON", "application/ json,charset=utf8", delParam, function(){layer.msg('已删除!',{icon:1,time:1000});document.location.reload();}, function(){layer.msg('删除失败!',{icon:1,time:1000});});
	});
	} else {
