...
function ajax_success(resp){
      var data = resp.jobTemlateParam;
     $("#paramNameCn").val(data.paramNameCn);
    $("#paramNameEn").val(data.paramNameEn);
    $("#paramValue").val(data.paramValue);
    $("#nextParamValue").val(data.nextParamValue);
...
$("#saveBtn").click(function () {
   if (validform().form()){
       ...
       var content={paramNameCn:paramNameCn,paramNameEn:paramNameEn,paramValue:paramValue,nextParamValue:nextParamValue,paramCondition:paramCondition,id:id}
       var data = {
            type:"1",
            param: content,
            action:"2"
