...
function loadJson(json) {
    if (json.result) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            // var data = json.result.jobTemplateFieldList[i];
            jobTemplateId = result.jobTemplateId;
            $("#agent").append("<tr class='text-c'>" +
                "<tr style='height: 60px'>" +
                "<th ><input type='checkbox' name='wholeCheckBox' name='templateResult' value=" + result["id"] + " id='" + result.id + "'></th>" +
        ...
        if (inputs.length) {
            ...
            for (var i = 0; i < inputs.length; i++) {
                ...
                if (inputs[i].checked == true) {
                    str += $(inputs[i]).attr("id") + ',';
                }
            }
            str = str.substring(0, str.length - 1);
        }
        var str_json = {"taskJobId": taskJobId, "jobTemplateId": jobTemplateId, "fieldIds": str, "type": "0"}
...
function mould2() {
    ...
    // function successMould(resp) {
    //     data = resp.result.taskJob;
    //     if (data.databaseId) {
    //         if (data.taskJobTimerId && proxytype) {
    //             window.location.href = window.history.go(-6);
    //         } else if (data.taskJobTimerId || proxytype) {
    //             window.location.href = window.history.go(-5);
    //         } else {
    //             window.location.href = window.history.go(-4);
    //         }
    //     } else if (data.taskJobTimerId) {
    //         if (data.proxyId) {
    //             window.location.href = window.history.go(-5);
    //         } else {
    //             window.location.href = window.history.go(-4);
...
function mould() {
    ...
    // }, error);
     window.location.href = "/use/list.html?username=dGVzdA==&password=MTIzNDU2";
    //  window.location.herf = "/use/list.html";
}
