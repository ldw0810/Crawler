...
$("#nextBtn").click(function () {
    if (workId) {
        str_json = {"action": "2", "delFlag": false, "status": "0", "id": workId, "proxyId": proxyId ,"taskJobTimerId": self.timerJobTimerId,};
    } else {
        str_json = {
            "action": "0",
            "delFlag": false,
...
function insertSuccess(res) {
    if (res.returnMsg == 'Task name already exist!') {
        ...
        window.location.href = "/use/mould.html?parentId=" + parentId + "&" + "proxytype=" + proxytype;
        workId = parentId;
    }
}
