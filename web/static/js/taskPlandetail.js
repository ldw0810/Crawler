...
function selfDetail(self_job,databaseSource){
    ...
    $("#detail_type").text(showType(self_job["type"]));
     if(ifUndefined(self_job["status"]) == 2){
            statues_content = "<span>运行中</span>";
        }else if(ifUndefined(self_job["status"]) == 1){
        ...
        }else if(ifUndefined(self_job["status"]) == 6){
...
function showFather(parent){
    if(parent){
        ...
        var parentContent = '<tr>' +
                        '<td>'+ ifUndefined(parent['name']) + '</td><td>' + ifUndefined(parent['url']) + '</td><td>' +ifUndefined(parent['type']) +
                        '</td><td>' + statues_content + '</td><td>' + ifUndefined(parent['grapInterval']) + '</td><td>' + ifUndefined(parent['grapPeriod']) +
    ...
    if(brotherList){
        ...
        for(var i = 0;i<brotherList.length;i++){
            ...
            var brotherContent = '<tr>' +
                                    ...
                                    '<td>' + ifUndefined(brotherList[i]['url']) + '</td><td>' + ifUndefined(brotherList[i]['type']) + '</td>' +
                                    '<td>' + statues_content + '</td>' +
                                    '<td>' + ifUndefined(brotherList[i]['grapInterval']) + '</td>' +
                                    '<td>' + ifUndefined(brotherList[i]['grapPeriod']) + '</td>' +
        ...
        for(var i = 0;i<childrenList.length;i++){
            ...
            var childrenContent = '<tr>' +
                                    ...
                                    '<td>' + ifUndefined(childrenList[i]['url']) + '</td>' +
                                     '<td>' + ifUndefined(childrenList[i]['type']) + '</td>' +
                                    '<td>' + statues_content + '</td>' +
                                    '<td>' + ifUndefined(childrenList[i]['grapInterval']) + '</td>' +
                                    '<td>' + ifUndefined(childrenList[i]['grapPeriod']) + '</td>' +
    ...
    if(list){
        ...
        for(var i = 0;i<list.length;i++){
            ...
            var content = '<tr>' +
                                    ...
                                    '</td><td>' + statues_content + '</td><td>' + ifUndefined(list[i]['grapInterval']) + '</td><td>' + ifUndefined(list[i]['grapPeriod']) +
